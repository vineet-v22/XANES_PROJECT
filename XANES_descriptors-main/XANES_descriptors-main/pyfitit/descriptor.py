...
def getQuality_SVM_linear1(sample, features, label_names, makeMixtureParams=None, model_class=None, model_regr=None, m=1, cv_count=10, returnModels=False, printDebug=True):
    """Get cross validation quality
...
def getQuality_SVM_linear2(sample, features, label_names, makeMixtureParams=None, model_class=None, model_regr=None, m=1, cv_count=10, returnModels=False, printDebug=True):
    """Get cross validation quality
...
def getQuality_SVM_rbf1(sample, features, label_names, makeMixtureParams=None, model_class=None, model_regr=None, m=1, cv_count=10, returnModels=False, printDebug=True):
    """Get cross validation quality
...
def getQuality_SVM_rbf2(sample, features, label_names, makeMixtureParams=None, model_class=None, model_regr=None, m=1, cv_count=10, returnModels=False, printDebug=True):
    """Get cross validation quality
...
def getQuality_SVM_pol_3(sample, features, label_names, makeMixtureParams=None, model_class=None, model_regr=None, m=1, cv_count=10, returnModels=False, printDebug=True):
    """Get cross validation quality
...
def getQuality_SVM_pol_2(sample, features, label_names, makeMixtureParams=None, model_class=None, model_regr=None, m=1, cv_count=10, returnModels=False, printDebug=True):
    """Get cross validation quality
    ...
    for il, label in enumerate(label_names):
        ...
        if model0 is None:
            ...
            for modelParams, j in zip(tryParams, range(len(tryParams))):
                if classification:
                    try_model[j] = SVC(kernel='poly', degree=2, **modelParams)
                else:
                    try_model[j] = SVR(kernel='poly', degree=2, **modelParams)
                if mix:
                    try_acc_mix, trueVals, pred, mod = mixture.score_cv(try_model[j], sample, features, label, label_names, makeMixtureParams, testRatio=0.5, repetitions=1, model_class=SVC(kernel='poly', degree=2))
                    try_acc[j] = try_acc_mix['avgLabels']
                else:
                    try_acc[j], pred = ML.score_cv(try_model[j], X, y, cv_count)
                    trueVals = y
            ...
            acc[0] = try_acc[bestj]
            model = try_model[bestj]
            model_class = SVC(kernel='poly', degree=2, **tryParams[bestj])
            if printDebug and len(tryParams)>1:
                print('Best model params: ', tryParams[bestj], f'Delta ={try_acc[bestj]-np.min(try_acc)}')
...
def getQuality_ANN5(sample, features, label_names, makeMixtureParams=None, m=1, cv_count=10, printDebug=True):
    quality = {}
    quality_std = {}
    predictedLabels = {}
    trueLabels = {}
    ...
    for il, label in enumerate(label_names):
        ...
        model = tf.keras.Sequential([
                tf.keras.layers.Dense(38, activation='relu', input_shape=(X_train.shape[1],)),
...
def getQuality_LogisticRegression(sample, features, label_names, makeMixtureParams=None, m=1, cv_count=10, printDebug=True):
    quality = {}
    quality_std = {}
    predictedLabels = {}
    trueLabels = {}
    ...
    for il, label in enumerate(label_names):
        ...
        for i in range(m):
            ...
            if classification:
                ...
                # You can't use accuracy score for regression, so leaving it as None
                acc.append(None)
...
def getQuality_ANN6(sample, features, label_names, makeMixtureParams=None, m=1, cv_count=10, printDebug=True):
    quality = {}
    quality_std = {}
    predictedLabels = {}
    trueLabels = {}
...
def getQuality_ANN7(sample, features, label_names, makeMixtureParams=None, m=1, cv_count=10, printDebug=True):
    quality = {}
    quality_std = {}
    predictedLabels = {}
    trueLabels = {}
    ...
    for il, label in enumerate(label_names):
        ...
        for i in range(m):
            ...
            model.compile(loss='mean_squared_error', optimizer=tf.keras.optimizers.Adam(learning_rate=0.001, beta_1=0.9, beta_2=0.999, epsilon=1e-07, amsgrad=False))
            model.fit(X_train, y_train, epochs=10, batch_size=32, verbose=0)
            y_pred = model.predict(X_test)
...
def getQuality_ANN8(sample, features, label_names, makeMixtureParams=None, m=1, cv_count=10, printDebug=True):
    quality = {}
    quality_std = {}
    predictedLabels = {}
    trueLabels = {}
...
def plot_descriptors_ANN8(data, descriptor_names, label_names, labelMaps=None, folder_prefix='', unknown=None, markersize=500, textsize=8, alpha=1, cv_count=2, plot_only='', doNotPlotRemoteCount=0, textColumn=None, additionalMapPlotFunc=None):
    """Plot 2d prediction map.
    ...
    os.makedirs(folder2, exist_ok=True)
    if plot_only != 'data':
        qualityRes = getQuality_ANN8(data, descriptor_names, label_names, m=1, cv_count=cv_count)
